...
{
    /* 変換前のレジスタ snapshot */
    uint32_t cr_before  = ADC1->CR;
    uint32_t isr_before = ADC1->ISR;

    /* TEMP チャンネルを単発読み (adc_read と同一手順) */
    ADC_ChannelConfTypeDef cfg = {0};
    cfg.Channel      = TRIAC_ADC_TEMP_CH;
    cfg.Rank         = ADC_REGULAR_RANK_1;
    cfg.SamplingTime = ADC_SAMPLETIME_391CYCLES;
    cfg.SingleDiff   = ADC_SINGLE_ENDED;
    cfg.OffsetNumber = ADC_OFFSET_NONE;
    ...
    uint32_t cr_after_start    = ADC1->CR;
    uint32_t isr_after_start   = ADC1->ISR;

    HAL_StatusTypeDef st_poll = HAL_ADC_PollForConversion(&s_hadc1, 10);
    uint32_t cr_after_poll    = ADC1->CR;
    uint32_t isr_after_poll   = ADC1->ISR;
    uint32_t dr_direct        = ADC1->DR;
    uint16_t hal_val          = (uint16_t)HAL_ADC_GetValue(&s_hadc1);

    char buf[120];
    snprintf(buf, sizeof(buf),
             "ADC_DIAG start=%d poll=%d hal=%u dr=%lu "
             "cr0=%08lX isr0=%08lX "
             "cr1=%08lX isr1=%08lX "
             "cr2=%08lX isr2=%08lX",
             (int)st_start, (int)st_poll,
             (unsigned)hal_val, (unsigned long)dr_direct,
             (unsigned long)cr_before,  (unsigned long)isr_before,
             (unsigned long)cr_after_start, (unsigned long)isr_after_start,
             (unsigned long)cr_after_poll,  (unsigned long)isr_after_poll);

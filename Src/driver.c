...
#warning Interrupt enabled input pins must have unique pin numbers! (suppressed for U585 port)
#endif
...
#endif
            default:
                // encoder_pin_claimed(aux_ctrl->port, pin); // disabled: no encoder
                break;
        }
    }
...
void gpio_irq_enable (const input_signal_t *input, pin_irq_mode_t irq_mode)
{
    if(irq_mode == IRQ_Mode_Rising) {
        exti_rtsr_set(input->bit);
        exti_ftsr_clr(input->bit);
    } else if(irq_mode == IRQ_Mode_Falling) {
        exti_rtsr_clr(input->bit);
        exti_ftsr_set(input->bit);
    } else if(irq_mode == IRQ_Mode_Change) {
        exti_rtsr_set(input->bit);
        exti_ftsr_set(input->bit);
    } else
        exti_imr_clr(input->bit);   // Disable pin interrupt

    if(irq_mode != IRQ_Mode_None)
        exti_imr_set(input->bit);    // Enable pin interrupt
}
...
void settings_changed (settings_t *settings, settings_changed_flags_t changed)
{
    ...
    if(IOInitDone) {
        ...
        for(int _p=5;_p<=9;_p++) HAL_NVIC_DisableIRQ(exti_irqn(_p));
#endif
#if (DRIVER_IRQMASK|AUXINPUT_MASK) & 0xFC00
        for(int _p=10;_p<=15;_p++) HAL_NVIC_DisableIRQ(exti_irqn(_p));
#endif
        ...
        do {
            ...
            if(input->group == PinGroup_AuxInput) {
                if(input->cap.irq_mode != IRQ_Mode_None) {
                    ...
                    uint32_t extireg = EXTI->EXTICR[input->pin >> 2] & ~(0b1111 << ((input->pin & 0b11) << 2));
                    extireg |= ((uint32_t)(GPIO_GET_INDEX(input->port)) << ((input->pin & 0b11) << 2));
                    EXTI->EXTICR[input->pin >> 2] = extireg;
                }
            }
        ...
        if(irq_mask & 0x03E0) {
            HAL_NVIC_SetPriority(exti_irqn(input->pin), 2, 0);
            HAL_NVIC_EnableIRQ(exti_irqn(input->pin));
        }
        if(irq_mask & 0xFC00) {
            HAL_NVIC_SetPriority(exti_irqn(input->pin), 2, 0);
            HAL_NVIC_EnableIRQ(exti_irqn(input->pin));
...
bool driver_init (void)
{
 ...
 // Drive MPG mode input pin low until setup complete
    ...
    // STM32U585: APB1プリスケーラ=1のため TIM5CLK = PCLK1（F4系の ×2 は不要）。
    // 元: HAL_RCC_GetPCLK1Freq() * 2 / STEPPER_TIMER_DIV  ← ×2でf_step_timerが2倍になり全速度が50%化していた
...
#endif
    exti_imr_set(input->bit); // Reenable pin interrupt
}

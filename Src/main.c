...
int main (void)
{
    ...
    /* TZEN=1対応: SAU無効化でメモリ全域を非セキュアに設定
     * Zephyrなしで起動するとCPUがセキュアモードで開始し、SAU未設定で
     * SecureFaultが発生してLockupする。HAL_Init()より前に実行すること。
     * TZEN=0環境では無害。
     * SAUシンボルはTZ無効ビルドでは未定義のため生アドレスで直接書込み。
     * SAU_BASE = 0xE000EDD0, CTRL offset = 0x00, ALLNS bit = bit1 */
    *((volatile uint32_t*)0xE000EDD0UL) = 0x00000002UL;  /* SAU->CTRL: ALLNS=1 */
    __DSB();
    __ISB();
#endif
    ...
    HAL_Init();  /* HAL_Init() は必ず最初に呼ぶこと */

    /* DBGMCUデバッグクロックを有効化
     * これがないとWFI中にOpenOCDがCPUにアクセスできなくなる（CPUID=0） */
    ...
    /* Range1: 160MHz 動作の必須条件 (HAL_Init()の後に呼ぶこと) */
    ...
    /* LPUART1クロックソースをPCLK3(160MHz)に設定
     * BRR = 256 * 160,000,000 / 115200 = 0x56CE4
     * ※ driver.c の LPUART1 init より前に呼ぶこと */
    RCC_PeriphCLKInitTypeDef PeriphClkInit_U5 = {
        .PeriphClockSelection  = RCC_PERIPHCLK_LPUART1,
        .Lpuart1ClockSelection = RCC_LPUART1CLKSOURCE_PCLK3,
    };
    if (HAL_RCCEx_PeriphCLKConfig(&PeriphClkInit_U5) != HAL_OK) {
        Error_Handler();
    }

    /* VDDIO2有効化: Port G (PG7=LPUART1_TX, PG8=LPUART1_RX) の電源隔離解除
     * STM32U585はデフォルトでPort Gが隔離されており、これなしにPG7/PG8は動作しない
     * MX_GPIO_Init()より前に呼ぶこと */
    HAL_PWREx_EnableVddIO2();  /* returns void */

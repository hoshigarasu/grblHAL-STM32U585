/*
  ...
  serial.c - serial port implementation for STM32F4xx/STM32U5xx ARM processors
...
#elif (SERIAL_PORT >= 90 && SERIAL_PORT < 99)
/* LPUART1 on STM32U5xx (UNO Q internal QRB2210 link: PG7=TX, PG8=RX) */
#define UART0            LPUART1
#define UART0_IRQ        LPUART1_IRQn
#define UART0_IRQHandler LPUART1_IRQHandler
#define UART0_CLK_En     __HAL_RCC_LPUART1_CLK_ENABLE
#else
...
#elif (SERIAL_PORT >= 90 && SERIAL_PORT < 99)
/* LPUART1 is clocked from PCLK3 (set to 160 MHz in main.c) */
#define UART0_CLK HAL_RCC_GetPCLK3Freq()
#else
...
#elif SERIAL_PORT == 90
/* LPUART1: PG7=TX(AF8), PG8=RX(AF8) - UNO Q internal QRB2210 link */
#define UART0_TX_PIN 7
#define UART0_RX_PIN 8
#define UART0_PORT   GPIOG
#define UART0_AF     GPIO_AF8_LPUART1
#else
...
/* ----------------------------------------------------------------
 * STM32U5 USART/LPUART register compatibility macros
 *
 * STM32F4: UART->SR / UART->DR  (single register for read and write)
 * STM32U5: UART->ISR / UART->RDR (read) / UART->TDR (write)
 * Interrupt bit names differ but values (bit positions) are identical:
 *   USART_SR_RXNE    == USART_ISR_RXNE_RXFNE  (bit 5)
 *   USART_SR_TXE     == USART_ISR_TXE_TXFNF   (bit 7)
 *   USART_SR_TC      == USART_ISR_TC           (bit 6)
 *   USART_CR1_TXEIE  == USART_CR1_TXEIE_TXFNFIE  (bit 7)
 *   USART_CR1_RXNEIE == USART_CR1_RXNEIE_RXFNEIE (bit 5)
 * ---------------------------------------------------------------- */
#if defined(STM32U585xx)
#define _U_SR(u)          ((u)->ISR)
#define _U_RDR(u)         ((u)->RDR)
#define _U_TDR(u)         ((u)->TDR)
/* SR bit name aliases (same bit positions as STM32F4 SR, different names in stm32u585xx.h)
 * USART_CR1_TXEIE / USART_CR1_RXNEIE are already defined in stm32u585xx.h */
#define USART_SR_RXNE     USART_ISR_RXNE_RXFNE
#define USART_SR_TXE      USART_ISR_TXE_TXFNF
#define USART_SR_TC       USART_ISR_TC
#else
#define _U_SR(u)          ((u)->SR)
#define _U_RDR(u)         ((u)->DR)
#define _U_TDR(u)         ((u)->DR)
#endif

/* Prevent conflict when a board map copies SERIAL_PORT into SERIAL1_PORT.
 * UNO Q uses only the internal QRB2210 link (LPUART1); no second port needed. */
#if defined(SERIAL1_PORT) && (SERIAL1_PORT == SERIAL_PORT)
#undef SERIAL1_PORT
#define SERIAL1_PORT 0
#endif
...
static uint16_t serialTxCount (void)
{
    ...
    return BUFCOUNT(head, tail, TX_BUFFER_SIZE) + (_U_SR(UART0) & USART_SR_TC ? 0 : 1);
}
...
#if (SERIAL_PORT >= 90 && SERIAL_PORT < 99)
    /* LPUART BRR = 256 * CLK / baud  (different formula from standard USART) */
    UART0->BRR = (uint32_t)(256ULL * UART0_CLK / baud_rate);
#else
...
#if (SERIAL_PORT >= 90 && SERIAL_PORT < 99)
        /* STM32U585: PG7/PG8 belong to Port G, which is power-isolated by default.
         * Must release VDDIO2 isolation before configuring GPIO.
         * HAL_PWREx_EnableVddIO2() returns void; no return value check needed. */
...
{
    if(_U_SR(UART0) & USART_SR_RXNE) {
        uint32_t data = _U_RDR(UART0);
    ...
    if((_U_SR(UART0) & USART_SR_TXE) && (UART0->CR1 & USART_CR1_TXEIE)) {
        uint_fast16_t tail = txbuf.tail;            // Get buffer pointer
        _U_TDR(UART0) = txbuf.data[tail];               // Send next character
